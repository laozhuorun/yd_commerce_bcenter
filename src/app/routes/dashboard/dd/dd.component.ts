...
export class DashboardDDComponent implements AfterViewInit, OnInit, OnDestroy {
  ...
  @ViewChild(CdkDropList) private placeholder: CdkDropList;
  @ViewChildren(DashboardDDContainerComponent) private containers: QueryList<
    DashboardDDContainerComponent
  >;
  private target: CdkDropList = null;
  private targetIndex: number;
  private source: CdkDropListContainer = null;
  ...
  enter = (drag: CdkDrag, drop: CdkDropList) => {
    ...
    if (dragIndex === -1) {
      dragIndex = Array.prototype.indexOf.call(
        dropEl.parentNode.children,
        phEl,
      );
    }
    ...
    dropEl.parentNode.insertBefore(
      phEl,
      dragIndex < dropIndex ? dropEl.nextSibling : dropEl,
    );
  ...
  showSettings() {
    this.modal
      .createStatic(DashboardDDSettingsComponent)
      .subscribe((res) => {
        this.widgets = res;
