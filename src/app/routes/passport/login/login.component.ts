...
export class UserLoginComponent extends FormComponentBase implements OnDestroy {
  form: FormGroup;
  error = '';
  type = 0;
  ...
  submit() {
    ...
    this.loading = true;
    this.loginService.authenticateModel.loginCertificate = this.userName.value;
    this.loginService.authenticateModel.password = this.password.value;
    this.loginService.authenticate(() => (this.loading = false));
