...
export class WareEditService implements OnDestroy {
  ...
  skuCho() {
    ...
    const validCount = selData.filter(w => w.filter(a => a.sel).length > 0)
      .length;
    ...
    const inFn = (depth: number) => {
      for (let i = 0; i < selData[depth].length; i++) {
        ...
        if (depth === selData.length - 1) {
          // 有效数据及去重
          if (
            tmpIds.filter(w => w !== 0).length === validCount &&
            !ids.includes(tmpIds.join(':'))
          ) {
    ...
    ids.forEach(attributes => {
      const item = Object.assign({}, this.tmpDef[attributes], {
        attributes,
        names: attributes
          .split(':')
          .map(v => +v)
          .map((id, idx) =>
            id > 0 ? this.cog.list[idx].arr.find(w => w.id === id).value : '',
          ),
      });
    ...
    if (this.i.skus.length === 0) {
      this.i.skus.push(Object.assign({}, this.tmpDef['']));
    }
  }
